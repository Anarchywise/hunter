...
class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
     ...
     *JwtAccessTokenConverter：
        ...
        这是一个用于将OAuth2访问令牌转换为JWT的组件。
        它负责对JWT进行签名和验证。

    KeyStoreKeyFactory：

        这是一个Spring Security类，用于从JKS（Java KeyStore）文件中加载密钥对。
        需要提供密钥库的位置（文件路径）、密钥库的访问密码（密钥）以及密钥对的别名和密码。

    KeyPair：

        一个KeyPair包含一个公钥和一个私钥。公钥用于验证JWT的签名，私钥用于生成签名。
        从密钥库中加载的KeyPair提供了JWT签名和验证所需的密钥。

    CustomUserAuthenticationConverter：

        这是一个自定义的用户身份转换器，用于将用户信息添加到JWT中。
        在这里，DefaultAccessTokenConverter的用户转换器被替换为自定义的CustomUserAuthenticationConverter。 
     * 
     * 
     */
    @Bean
